...
// When you click to go to the next page in a thread, point to the next reply,
// not the top of the page, since the first post of the thread is always shown.
simplePrefs.on("topPostRedirect", function(pref) {
  if(simplePrefs.prefs[pref]) {
  ...
  if(message.split("::")[0] == "RTSE") {
    ...
    switch(message.split("::")[1]) {
      ...
      case "NextPageRequest":
        if(simplePrefs.prefs["topPostRedirect"])
          this.postMessage("RTSENEXTPAGERESPONSE::YES");
        break;
